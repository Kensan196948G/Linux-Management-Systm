...
Webmin風 Linux管理WebUIは、Linuxサーバの運用管理をWebブラウザから安全に行うためのシステムです。
...
| 機能 | 内容 |
|------|------|
| システム状態 | CPU / メモリ / ディスク使用率 |
| サービス状態 | systemctl status 取得 |
| プロセス一覧 | ps / top（参照のみ） |
...
|------|------|
| 操作ログ | 誰が・いつ・何を |
| 実行結果記録 | stdout / stderr |
| 操作区分 | 参照 / 実行 |
| ログ保全 | 追記専用・改ざん防止 |
| 監査出力 | CSV / JSON エクスポート |
